...
new_data <- hyd_dep %>%
  ...
  mutate(prob_fit = predict(dep_ptch_mdl, newdata = hyd_dep)) #%>%
...
plot(new_data$Q, new_data$prob_fit, type="n", main = "Adult Patch/Depth: Probability according to Q", xlab="Q (cfs)", ylab="Probability of Occurrence")
lines(spl, col="black")
points(newxmid, newymid, col="red", pch=19) # 50%
## if node is deeper then 2 points needed for limit

plot(new_data$depth_cm, new_data$prob_fit, main = "Adult Patch/Depth: Probability according to Depth (cm)", xlab="Depth (cm)", ylab="Probability of Occurrence")
