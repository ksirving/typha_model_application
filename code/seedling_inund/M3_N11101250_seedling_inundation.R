...
new_dataM <- hyd_depM %>%
  mutate(prob_fit = predict(dep_sdlng_mdl, newdata = hyd_depM, type="response")) %>%
  mutate(prob_fit = ifelse(prob_fit<=0, 0, prob_fit)) ## predicts negative percentages - cut off at 0 for quick fix

new_dataL <- hyd_depL %>%
  mutate(prob_fit = predict(dep_sdlng_mdl, newdata = hyd_depL, type="response")) %>%
  mutate(prob_fit = ifelse(prob_fit<=0, 0, prob_fit)) ## predicts negative percentages - cut off at 0 for quick fix

new_dataR <- hyd_depR %>%
  mutate(prob_fit = predict(dep_sdlng_mdl, newdata = hyd_depR, type="response")) %>%
  mutate(prob_fit = ifelse(prob_fit<=0, 0, prob_fit)) ## predicts negative percentages - cut off at 0 for quick fix
...
save(new_data, file="output_data/M3_N11101250_typha_seedling_depth_discharge_probability_time_series_red_columns.RData")
...
save(new_data, file="output_data/M3_N11101250_typha_seedling_depth_discharge_probs_2010_2017_TS.RData")
...
write.csv(limits, "output_data/M3_N11101250_Typha_seedling_Depth_Q_limits.csv")

# plot discharge points ---------------------------------------------------

png("Figures/Application_curves/Depth/N11101250_seedling_depth_prob_Q_thresholds.png", width = 500, height = 600)
...
write.csv(melt_time, "output_data/M3_N11101250_typha_seedling_depth_time_stats.csv")
...
png("Figures/Application_curves/Depth/N11101250_typha_seedling_depth_perc_time_above_threshold_annual.png", width = 500, height = 600)
...
png("Figures/Application_curves/Depth/N11101250_typha_seedling_depth_perc_time_above_threshold_non_critcal.png", width = 500, height = 600)
...
png("Figures/Application_curves/Depth/N11101250_typha_seedling_depth_perc_time_above_threshold_critcal.png", width = 500, height = 600)
...
### take info  about the thresholds and include it here - use the equations as objects
### e.g. sum(Q >= newx1a)/length(DateTime)*100
## low_equation <- Q >= newx1a
...
write.csv(total_days, "output_data/M3_N11101250_typha_seedling_depth_total_days.csv")
...
## save df
write.csv(melt_days, "output_data/M3_N11101250_typha_seedling_depth_total_days_long.csv")
...
## plot all ts
png("Figures/Application_curves/Depth/N11101250_typha_seedling_depth_lob_rob_mc_no_days_within_Q.png", width = 500, height = 600)
...
## plot by year
png("Figures/Application_curves/Depth/N11101250_typha_seedling_depth_lob_rob_mc_no_days_within_Q_by_year.png", width = 500, height = 600)
...
png("Figures/Application_curves/Depth/N11101250_typha_seedling_depth_lob_rob_mc_no_days_within_Q_by_season.png", width = 500, height = 600)
